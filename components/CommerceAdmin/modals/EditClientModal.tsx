...
import { getCustomer, updateCustomerAccount, type CustomerRow } from "@/services/customerService";
...
export default function EditClientModal({ open, client, onClose, onUpdated }: Props) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([detail, services, addons, assigned]) => {
        const assignedRows = Array.isArray(assigned) ? assigned : [];
        const assignedServices = assignedRows
          .filter((row) => String(row.category ?? "").toLowerCase() !== "add-on")
          .map((row) => row.title)
          .filter(Boolean);
        const assignedAddons = assignedRows
          .filter((row) => String(row.category ?? "").toLowerCase() === "add-on")
          .map((row) => row.title)
          .filter(Boolean);
        ...
        setForm({
          ...
          phone: detail?.phone ?? "",
          avatar: null,
          services: assignedServices.length ? assignedServices : [],
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const validationError = validateClientAccountForm(form, "edit");
    if (validationError) {
    ...
    if (form.services.length === 0) {
      toast.error("Please select at least one Active Service / Product.");
      return;
    }

    setSubmitting(true);
    try {
      await updateCustomerAccount(client.id, {
      ...
      toast.success(`Client account CL-${client.id} updated successfully!`);
      handleClose();
      onUpdated();
    } catch (err: any) {
      const message =
    ...
    <div className={styles.modalOverlay} role="dialog" aria-modal="true">
      <div className={styles.modalCardWide}>
        <div className={styles.modalHeader}>
          <div>
            <h3 className={styles.modalTitle}>Edit Customer Account</h3>
            <p className={styles.panelSubtitle}>
              CL-{client.id} · {client.name}
            </p>
          </div>
        ...
        {loadingDetail ? (
          <p className={styles.emptyState}>Loading client account...</p>
        ) : (
